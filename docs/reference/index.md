# Package index

## All functions

- [`angled2()`](https://groupmodes.github.io/controlDim/reference/angled2.md)
  : Cálculo de ángulos direccionales de vectores normalizados
- [`axis3()`](https://groupmodes.github.io/controlDim/reference/axis3.md)
  : Ejes 3D con control de la distancia de etiquetas y longitud de
  marcas
- [`bbox()`](https://groupmodes.github.io/controlDim/reference/bbox.md)
  : Calcular los límites espaciales (bounding box) de una malla 3D
- [`cleanMesh3d_rgl()`](https://groupmodes.github.io/controlDim/reference/cleanMesh3d_rgl.md)
  : Limpieza de una malla 3D
- [`deuclid()`](https://groupmodes.github.io/controlDim/reference/deuclid.md)
  : Calcular la distancia euclidiana entre dos puntos
- [`findModes()`](https://groupmodes.github.io/controlDim/reference/findModes.md)
  : Calcular modas de un conjunto de datos
- [`fplot3d()`](https://groupmodes.github.io/controlDim/reference/fplot3d.md)
  : Leyenda categórica en 3D
- [`fshade3d()`](https://groupmodes.github.io/controlDim/reference/fshade3d.md)
  : Representación 3D con colores categóricos
- [`getBoundarySegments3()`](https://groupmodes.github.io/controlDim/reference/getBoundarySegments3.md)
  : Extrae el borde de una malla tridimensional
- [`getview3d()`](https://groupmodes.github.io/controlDim/reference/getview3d.md)
  : Obtener la vista 3D actual
- [`pan3d()`](https://groupmodes.github.io/controlDim/reference/pan3d.md)
  : Desplazamiento interactivo de la vista 3D
- [`setview3d()`](https://groupmodes.github.io/controlDim/reference/setview3d.md)
  : Establecer una vista 3D
- [`sideBulb()`](https://groupmodes.github.io/controlDim/reference/sideBulb.md)
  : Determinar la orientación del bulbo de un refuerzo
- [`sortSegments2()`](https://groupmodes.github.io/controlDim/reference/sortSegments2.md)
  : Reordenar segmentos conectados de un borde
- [`splitTrianglesInd()`](https://groupmodes.github.io/controlDim/reference/splitTrianglesInd.md)
  : Divide una malla en grupos de triángulos conectados
- [`splot3d()`](https://groupmodes.github.io/controlDim/reference/splot3d.md)
  : Barra de colores continua en 3D
- [`sshade3d()`](https://groupmodes.github.io/controlDim/reference/sshade3d.md)
  : Representa una superficie 3D coloreada con leyenda
- [`surfaceDistance()`](https://groupmodes.github.io/controlDim/reference/surfaceDistance.md)
  : Calcular la distancia sobre una superficie 3D
