...
Determines whether the bulb of a reinforcement is oriented to the left
or right based on the vertex coordinates of a `mesh3d` object.
...
The reinforcement is divided into two regions using the midpoint of the
observed Z range:

\$\$z\_{mid} = \frac{z\_{min} + z\_{max}}{2}\$\$

Vertices above this value are considered part of the upper region
containing the bulb, while the remaining vertices define the lower
region.

The minimum X coordinate of both regions is then compared. If the upper
region extends farther toward smaller X values than the lower region,
the bulb is classified as left-oriented.
